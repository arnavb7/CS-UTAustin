 ...
 *  results here:
 * The runtime for the "Adding at end" operation is about the same for ArrayLists and
 * LinkedLists, with LinkedLists being slightly faster. I would expect the Big O to be O(N) based
 * on the timing data shown.
 *
 * The runtime for the "Adding at front" operation is faster for LinkedLists than for
 * ArrayLists. I would expect the Big O to be O(N^2) for ArrayLists and O(N) for LinkedLists
 * based on the timing data shown.
 *
 * The runtime for the "Removing from front" operation is faster for LinkedLists than for
 * ArrayLists. I would expect the Big O to be O(N^2) for ArrayLists and O(N) for LinkedLists
 * based on the timing data shown.
 *
 * The runtime for the "Getting random" operation is faster for ArrayLists than for
 * LinkedLists. I would expect the Big O to be O(N) for ArrayLists and O(N^2) for LinkedLists based
 * on the timing data shown.
 *
 * The runtime for the "Getting all using iterator" operation is about the same for ArrayLists and
 * LinkedLists, with ArrayLists being slightly faster. I would expect the Big O to be O(N) based
 * on the timing data shown.
 *
 * The runtime for the "Getting all using get method" operation is faster for ArrayLists than for
 * LinkedLists. I would expect the Big O to be O(N) for ArrayLists and O(N^2) for LinkedLists based
 * on the timing data shown.
 */
